...
public class PulsePowerMeterTest {
    ...
    public void getAveragePower_0ts() {
        assertEquals(0, pulsePowerMeter.getAveragePower(now));
    }
    ...
    public void getAveragePower_2ts() {
        ...
        assertEquals(1, pulsePowerMeter.getAveragePower(now));
    }
...
//        assertEquals(4347, pulsePowerMeter.getAveragePower(now));
//    }
    ...
    public void getAveragePower_3ts_2tsi() {
        ...
        assertEquals(120, pulsePowerMeter.getAveragePower(now));
    }

    @Test
    public void getAveragePower_4ts_3tsi() {
        addTimestampSeconds(-999, -60, -50, -30);
        assertEquals(270, pulsePowerMeter.getAveragePower(now));
    }

    @Test
    public void getMinPower_4ts_3tsi() {
        addTimestampSeconds(-999, -60, -50, -30);
        assertEquals(180, pulsePowerMeter.getMinPower(now));
    }

    @Test
    public void getMaxPower_4ts_3tsi() {
        addTimestampSeconds(-999, -60, -50, -30);
        assertEquals(360, pulsePowerMeter.getMaxPower(now));
    }

    private void addTimestampSeconds(int... seconds) {
        for(int i=0; i<seconds.length ;i++) {
            pulsePowerMeter.addTimestamp(now.plusSeconds(seconds[i]));
        }
    }

    private void addTimestampMillis(int... millis) {
        for(int i=0; i<millis.length ;i++) {
            pulsePowerMeter.addTimestamp(now.plusNanos(millis[i] * 1000000));
        }
    }
}
