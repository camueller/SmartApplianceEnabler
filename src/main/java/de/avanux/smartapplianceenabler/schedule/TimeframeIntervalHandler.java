...
//                            activateTimeframeInterval(event.getTime(), timeframeInterval);
    ...
    public void updateQueue(LocalDateTime now, Integer considerTimeframeIntervalsActivatableWithinSeconds) {
        ...
        if((deactivatableTimeframeInterval.isPresent() && deactivatableTimeframeInterval.get() == getFirstTimeframeInterval()) || activatableTimeframeInterval.isPresent()) {
    ...
    public boolean hasOptionalEnergyTimeframe(LocalDateTime now) {
        TimeframeInterval timeframeInterval = findOptionalEnergyIntervalForEVCharger();
        if(timeframeInterval != null) {
            return !timeframeInterval.getRequest().isFinished(now);
        }
        return false;
    }
