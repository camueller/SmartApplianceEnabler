...
Ist ein *Standardwert für Überschussenergie* gesetzt, wird nur bis zu diesem Wert mit Überschussenergie geladen und danach der Ladevorgang gestoppt.

Die vom *Smart Appliance Enabler* unterstützten Wechselstrom-Wallboxen können nicht den aktuellen Ist-Ladezustand vom Fahrzeug ermitteln und an den *Smart Appliance Enabler* kommunizieren! Für eine möglichst genaue Ermittlung des Energiebedarfs muss dieser Wert aber bekannt sein. Der *Smart Appliance Enabler* bietet deshalb die Möglichkeit zum [automatisierten Auslesen des SOC](soc/SOC_DE.md).

Wenn mehrere Fahrzeuge konfiguriert werden, ist eine automatische Erkennung des mit der Wallbox verbundenen Fahrzeugs nur möglich, wenn beim [automatisierten Auslesen des SOC](soc/SOC_DE.md) neben dem SOC auch mindestens einen der folgenden Werte liefern kann:

- Verbindungsstatus des Ladekabels
- Zeitpunkt des Einsteckens des Ladekabels
- Geo. Breite/Länge des Fahrzeugs

Falls vorhanden, müssen diese Werte mit [Regulären Ausdrücken(Regex)](ValueExtraction_DE.md) extrahiert werden.
...
## Ladevorgang
Wenn das [automatisierten Auslesen des SOC](soc/SOC_DE.md) konfiguriert wurde, wird dieses **automatisch nach dem Verbinden des Fahrzeuges mit der Wallbox** ausgeführt.
...
- [automatisiert ausgelesenem SOC](soc/SOC_DE.md) oder über die [Ampel-Steuerung](Status_DE.md#user-content-click-green-ev) eingegebenem `Ist-SOC`,
...
- `letzter bekannter SOC`: [automatisiert ausgelesenem SOC](soc/SOC_DE.md) (Ladebeginn bzw. letzte Ausführung) bzw. Eingabe via Ampel (bei Ladebeginn)
- `bezogene Energie`:  gemessen vom Zähler
- `Batteriekapazität`: aus der Fahrzeug-Konfiguration
- `Ladeverluste`: initial aus der Fahrzeug-Konfiguration; bei aktivierter Ausführungswiederholung des [automatisierten Auslesens des SOC](soc/SOC_DE.md) werden die tatsächlichen Ladeverluste berechnet und verwendet
...
Ist die **Ausführungswiederholung** für das [automatisierten Auslesen des SOC](soc/SOC_DE.md) aktiviert, wird dies während des Ladevorgangs das **periodisch ausgeführt**. Wenn sich der berechnete SOC entweder um einen konfigurierten Wert (Standard: 20%) erhöht oder seit der letzten Ausführung eine konfigurierte Zeit vergangen ist, erfolgt eine erneute Ausführung. Der berechnete SOC wird mit dem tatsächlichen SOC verglichen und daraus die tatsächlichen Ladeverluste berechnet. Für alle nachfolgenden Berechnungen des SOC bis zur nächsten Ausführung während des aktuellen Ladevorganges werden die tatsächlichen Ladeverluste berücksichtigt.

**Ohne [automatisiertes Auslesen des SOC](soc/SOC_DE.md)** und ohne [Eingabe des aktuellen Ist-Ladezustands](Status_DE.md#user-content-click-green-ev) geht der *Smart Appliance Enabler* von einem Ist-Ladezustand von 0% aus wenn das Fahrzeug verbunden wird und meldet dem *Sunny Home Manager* einen entsprechend großen Energiebedarf. Das verschlechtert zwar dessen Planung, aber unabhängig davon beendet die Wallbox das Laden spätestens, wenn das Fahrzeug voll geladen ist.
...
... wurde durch [automatisiertes Auslesen des SOC](soc/SOC_DE.md) dessen Wert mit 51% bestimmt:
...
Der Ladevorgang ist vorangeschritten und der berechnete SOC liegt um 20% über dem SOC bei der letzten Ausführung des [automatisiertes Auslesens des SOC](soc/SOC_DE.md) (Standardwert der Konfiguration) - also ist eine erneute Ausführung notwendig:
...
Der berechnete SOC ist 71%, der durch [automatisiertes Auslesen des SOC](soc/SOC_DE.md) gelieferte Wert ist 72%. Aus der SOC-Änderung läßt sich die Energie berechnen, die im Fahrzeug "angekommen" ist (`energyReceivedByEv`). Aus dieser und der laut Zähler dafür aufgewendeten Energiemenge lassen sich die tatsächlichen Ladeverluste berechnen - hier 4%:
...
Der berechnete SOC liegt abermals 20% über dem SOC der letzten [automatisierten Auslesung des SOC](soc/SOC_DE.md) - also ist eine erneute Ausführung notwendig:
...
Der berechnete SOC war 92%, der mittels [automatisierter Auslesung des SOC](soc/SOC_DE.md) gelieferte Wert war 90%. Die Berechnung der tatsächlichen Ladeverluste ergibt 14%:
...
Die berechnete Restmenge der benötigen Energie zur Erreichung des Soll-SOC ist erstmals kleiner 1 kWh - deshalb wird eine weitere Ausführung der [automatisierten Auslesung des SOC](soc/SOC_DE.md) angestossen um sicherzustellen, dass das Fahrzeug tatsächlich kurz davor ist, den Soll-SOC zu erreichen:
...
Der berechnete SOC ist 99%, der von der [automatisierten Auslesung des SOC](soc/SOC_DE.md) gelieferte Wert ist 98%. Daraus lassen sich die Ladeverluste berechnen: 23% in dieser Phase des Ladens:
...
### Automatisiertes Auslesen des SOC
Für jede Ausführung des [automatisierten Auslesens des SOC](soc/SOC_DE.md) finden sich im [Log](Logging_DE.md) folgende Zeilen:
